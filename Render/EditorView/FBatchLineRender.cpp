...
void FBatchLineRenderer::Initialize(ID3D11Device* InDevice, uint32 InitialLineCapacity)
{
	...
	ErrorHandler::Report(!DepthTestedPipeline->InitializeFromFile(Device, "./Content/Pipeline/BatchLineDepthTested.json"), "[ FBatchLineRenderer ]", "Failed to initialize the depth-tested batch line pipeline.", ErrorHandler::EErrorLevel::Critical);
	ErrorHandler::Report(!OverlayPipeline->InitializeFromFile(Device, "./Content/Pipeline/BatchLineOverlay.json"), "[ FBatchLineRenderer ]", "Failed to initialize the overlay batch line pipeline.", ErrorHandler::EErrorLevel::Critical);
